...
class classes {
    ...
    metRequirement(currentClass: classes, remainingClasses: classes[]) {
        ...
        for (var requiredClasses of this.requirements) {
...
class semesterSchedule {
    ...
    to_string(): string {
        var temp = this.MonWedFri.toString() + this.TuTH.toString();
        return temp;
    }
...
class availableClasses {
    ...
    earliestTimeMatch(endTime: Date) { //endtime is the time the class ends
        var temp = new Date(0, 0, 23, 59);
        var tempClass;
        var test = new Date(0, 0, 11, 23);
        // console.log("Earliest Time Match");
        // console.log("the end time is "+endTime.toLocaleTimeString());
        for (var avaClass of this.availableClasses) {
...
function generateSchedule() {
    var ams101 = new classes("AMS101", 3, false, new Date(0, 0, 0, 8, 45), new Date(0, 0, 0, 9, 15)); //datetime might be fucked
    var ams102 = new classes("AMS102", 3, false, new Date(0, 0, 0, 10, 45), new Date(0, 0, 0, 11, 15));
    var ams103 = new classes("AMS103", 3, false, new Date(0, 0, 0, 10, 45), new Date(0, 0, 0, 11, 15));
    ams102.addRequirements(ams101);
    ams103.addRequirements(ams102);
    var cse101 = new classes("CSE101", 3, false, new Date(0, 0, 0, 12, 15), new Date(0, 0, 0, 13, 30));
    var cse102 = new classes("CSE102", 3, false, new Date(0, 0, 0, 14, 30), new Date(0, 0, 0, 15, 30));
    cse102.addRequirements(cse101);
    var ele101 = new classes("ELE101", 3, true, new Date(0, 0, 1, 17, 15), new Date(0, 0, 0, 18, 15));

    var allClass: classes[] = [ele101, ams102, ams103, cse101, cse102, ams101];

    var AvailableClasses = new availableClasses(allClass);
    var requirement = new majorRequirement([ams102, ams101, cse101, cse102], 1, 3);
    var semester1 = new semesterSchedule();

    //fill out the schedule

    var endTime = new Date(0, 0, 0, 0, 0);
    var filled = true;
    var tempClass: classes;
    var schedule:classes[] =[];
    while (true) {
...
function printSchedule(schedule:classes[]){
    var mwf = new Date(0,0,0,0,0);
    var tuth = new Date(0,0,1,0,0);
    console.log("Monday")
    for(var thisClass of schedule){
